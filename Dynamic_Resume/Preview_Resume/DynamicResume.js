...
window.addEventListener('load', () => {
    ...
    // Tour Vercel deploymentUrl
    const baseUrl = 'http://127.0.0.1:5501/Dynamic_Resume/Preview_Resume/DynamicResume.html';
    //    Create a unique URL
    const uniqueResumeUrl = `${baseUrl}?/${username}`;
    // Set the resume link in DOM
    const resumeLink = document.getElementById('resumeLink');
    resumeLink.setAttribute('href', uniqueResumeUrl);
    resumeLink.textContent = "Unique URL";
...
document.getElementById('ptintBtn').addEventListener('click', () => {
    window.print();
});
